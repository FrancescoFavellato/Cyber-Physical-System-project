function ris_opgd = OPGD(A, C, y, a, x_true, lambda, nu, T)
    % OUTPUT:
    % ris_opgd  - struct con campi:
    ...
    for k = 1:T
        ...
        tic
        CAk  = C * Apow;        % C * A^k (dove alla prima iterazione Apow = A^0)

        % Calcolo del residuo
        r = CAk * x0_hat + a_hat - yk;
        
        % Passo di aggiornamento su x0_hat
        x0_hat = x0_hat - nu * (CAk' * r);
    
        % Passo di aggiornamento su a_hat
        a_hat = a_hat - nu * r;
        a_hat = soft_threshold(a_hat, nu * lambda);
        ...
        x_true_k = x_true(:, k);            % x(k) vero = A^k * x(0)
    ...
    % Output
    ris_opgd.x0_hat = x0_hat;
    ris_opgd.a_hat = a_hat;
    ris_opgd.x_hat = x_hat;
    ris_opgd.state_err = state_err;
    ris_opgd.support_err = support_err;
    ris_opgd.false_pos = false_pos;
    ris_opgd.false_neg = false_neg;
    ris_opgd.time = tempo_complessivo;
end